...
import { uploadMeal, MealDto, createMealWithoutPhoto } from '../api';
...
export default function AddMeal() {
  ...
  const textOnlyMutation = useMutation({
    mutationFn: async () => {
      let created: Date | undefined = undefined;
      if (createdAtLocal) created = new Date(createdAtLocal);
      if (!description || !description.trim()) throw new Error('Please enter a description');
      return await createMealWithoutPhoto(created, description);
    },
    onSuccess: (meal: MealDto) => {
      qc.invalidateQueries({ queryKey: ['meals'] });
      qc.invalidateQueries({ queryKey: ['summary'] });
  ...
  const textOnlyBusy = textOnlyMutation.isPending;
  ...
  <label className="block text-sm font-medium">Photo</label>
        ...
        <p className="text-xs text-gray-500">No photo? You can log by description only — AI will still estimate macros.</p>
        <input
          ref={cameraInputRef}
          type="file"
          accept="image/*"
  ...
  <p className="text-xs text-gray-500">Adjust if logging a past meal.</p>
      ...
      <div className="space-y-1">
        ...
        <p className="text-xs text-gray-500">Tip #1: If you leave this blank, the AI will generate it from the photo.</p>
        <p className="text-xs text-gray-500">Tip #2: If you enter a brief description, it can improve analysis accuracy.</p>
        <p className="text-xs text-emerald-700">Forgot a photo? Enter a quick description and use "Log Without Photo" below.</p>
      </div>
      <div className="flex flex-col sm:flex-row gap-2">
        <button
          disabled={!file || busy}
        ...
        <button
          type="button"
          disabled={textOnlyBusy || !description.trim()}
          onClick={() => textOnlyMutation.mutate()}
          className="inline-flex items-center justify-center px-5 py-2.5 rounded-md bg-white text-emerald-700 font-semibold shadow-sm border border-emerald-200 hover:bg-emerald-50 focus:outline-none focus:ring-2 focus:ring-emerald-200 active:scale-[0.99] transition disabled:opacity-60 w-full sm:w-auto"
          aria-label="Log meal without photo"
          title="Log Without Photo"
        >
          {textOnlyBusy ? 'Logging…' : 'Log Without Photo'}
        </button>
      </div>
      {busy && (
      ...
      {textOnlyBusy && (
        <div className="flex items-center gap-2 text-sm text-emerald-700">
          <Spinner size={16} title="Submitting" />
      ...
      {textOnlyMutation.isError && <div className="text-sm text-red-600">Error: {(textOnlyMutation.error as any).message}</div>}
